...
<link rel="stylesheet" href="/RecoltePure/assets/css/categories.css">
    ...
    <section class="categories-section">
        ...
        <div class="category-list">
            <?php foreach($categories as $cat): ?>
                <a href="/RecoltePure/categories/<?= $cat['category_id']; ?>">


                    <div class="category-item">
                        <div class="cat-img-wrapper">
                             <img src="/RecoltePure/assets/uploads/products/Fruits.png" alt="Category">
                        </div>
    ...
    <div class="search-group">
        ...
        <button type="submit"><i class="fas fa-search"></i></button>
            ...
            <select class="sort-select" style="margin-left: 15px;" 
               onchange="window.location.href='/RecoltePure/categories/<?= $categoryId ?>?search=<?= urlencode($search) ?>&sort=' + this.value;">
...
<div class="product-card">
      <div class="product-img-wrapper"  data-images='<?= json_encode($images); ?>'>
            <img src="/RecoltePure/assets/uploads/products/<?= htmlspecialchars($firstImage); ?>" 
            ...
            <div class="product-footer">
               ...
               <form method="POST" action="/RecoltePure/cart">

                    <input type="hidden" name="product_id" value="<?= $product['product_id']; ?>">
                    <input type="hidden" name="product_name" value="<?= htmlspecialchars($product['product_name']); ?>">
                    <input type="hidden" name="price" value="<?= $product['price']; ?>">
                    <input type="hidden" name="quantity" class="quantity-input" value="1">
                    <input type="hidden" name="image" value="assets/uploads/products/<?= htmlspecialchars($firstImage); ?>">
...
<div class="pagination">
    <?php if ($page > 1): ?>
        <a class="page-btn" href="/RecoltePure/categories/<?= $categoryId ?>/<?= $search ? 'search/'.urlencode($search).'/' : '' ?>page/<?= $page - 1 ?>">Prev</a>
    <?php endif; ?>

    <?php for ($i = 1; $i <= $totalPages; $i++): ?>
        <a class="page-btn <?= ($i==$page)?'active':'' ?>" 
           href="/RecoltePure/categories/<?= $categoryId ?>/<?= $search ? 'search/'.urlencode($search).'/' : '' ?>page/<?= $i ?>">
           <?= $i ?>
        </a>
    <?php endfor; ?>

    <?php if ($page < $totalPages): ?>
        <a class="page-btn" href="/RecoltePure/categories/<?= $categoryId ?>/<?= $search ? 'search/'.urlencode($search).'/' : '' ?>page/<?= $page + 1 ?>">Next</a>
    <?php endif; ?>
</div>
